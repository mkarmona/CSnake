...
def closeOutputFile(output):
    output.close()
    if outputFileName[-4:] == ".tmp":
        prevOutputFilename = outputFileName[:-4]
        if os.path.exists(prevOutputFilename):
            f1 = open(prevOutputFilename, 'r')
            f2 = open(outputFileName, 'r')
            overwritePrev = f1.read() != f2.read()
            f1.close()
            f2.close()
            if overwritePrev:
                shutil.copy(outputFileName, prevOutputFilename)
            #os.unlink(prevOutputFilename)
